...
describe("resume selectors", (): void => {
  it("maps frame records to summaries", (): void => {
    const userId = "user-1" as FrameRecord["userId"];
    const lessonId = "lesson-1" as FrameRecord["lessonId"];
    const threadId = "thread-1" as FrameRecord["threadId"];

    const record: FrameRecord = {
      _id: "frame-1" as FrameRecord["_id"],
      userId,
      lessonId,
      videoTimeSec: 120,
      threadId,
  ...
  it("resolves frame upserts by overwriting fields", (): void => {
    const userId = "user-2" as FrameSummary["userId"];
    const lessonId = "lesson-2" as FrameSummary["lessonId"];
    const threadId = "thread-2" as FrameSummary["threadId"];

    const existing: FrameSummary = {
      id: "frame-2" as FrameSummary["id"],
      userId,
      lessonId,
      videoTimeSec: 15,
      threadId,
  ...
  it("maps code snapshots and overwrites values", (): void => {
    const userId = "user-3" as CodeSnapshotUpsertInput["userId"];
    const lessonId = "lesson-3" as CodeSnapshotUpsertInput["lessonId"];
    ...
    const summary = resolveCodeSnapshotSummary(
      "snapshot-1" as CodeSnapshotSummary["id"],
      input,
      3000,
    );
  ...
  it("maps code snapshot records to summaries", (): void => {
    const userId = "user-4" as CodeSnapshotRecord["userId"];
    const lessonId = "lesson-4" as CodeSnapshotRecord["lessonId"];

    const record: CodeSnapshotRecord = {
      _id: "snapshot-2" as CodeSnapshotRecord["_id"],
      userId,
      lessonId,
      language: "py",
      code: "print('hi')",
  ...
  it("builds code snapshot patches from inputs", (): void => {
    const input: CodeSnapshotUpsertInput = {
      userId: "user-5" as CodeSnapshotUpsertInput["userId"],
      lessonId: "lesson-5" as CodeSnapshotUpsertInput["lessonId"],
      language: "js",
      code: "console.log('ok')",
      codeHash: "hash-3",
    };
