...
- Initial content includes CS50x (2026), CS50P, CS50W, and CS50 AI (2023) delivered as YouTube-embedded lessons; lesson granularity follows source video structure.
- Watch embedded YouTube, code in notepad-style editor, and chat with Nio (strict TA tone - Prof. David Malan, Harvard, persona).
...
- Execution Packs: JS (Worker), TS (WASM transpile + Worker), Python (Pyodide Worker), HTML/CSS (sandboxed iframe), C via in-browser TCC pack; uniform executor interface init/run/stop; warm-up via idle prefetch/cache and “Preparing runtimes…” inline status.
...
- Environments/CI: Dev (local Next + Convex dev), Preview (Vercel Preview + Convex preview), Prod (Vercel Prod + Convex prod); Vercel build uses `npx convex deploy --cmd 'bun run build'`; E2E (Playwright) triggered on `vercel.deployment.success`; Sentry + Semgrep in CI; PWA minimal shell/cache last session (no offline AI/video).
...
Acceptance Criteria
- Language switch after warm-up feels instant; C “hello world” compile+run <500ms on modern laptop; Python/TS/JS warm-up hidden by background prefetch; switching Python→C after warm-up <100ms perceived delay or shows non-blocking warm-up notice.
...
- Theme toggle always visible; layouts selectable and persisted; light theme default; minimal chrome with monospace-forward typography.
