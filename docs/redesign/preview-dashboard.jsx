...
const Card = ({ children, className = '', hoverable = false }) => (
  <div className={`bg-[#232323] border border-[#2e2e2e] rounded-lg p-5 ${hoverable ? 'hover:border-[#404040] hover:shadow-lg transition-all duration-150 cursor-pointer' : ''} ${className}`}>
    {children}
  </div>
);
...
const Button = ({ children, variant = 'primary', size = 'md', className = '' }) => {
  const variants = {
    ...
    secondary: 'bg-[#2a2a2a] hover:bg-[#333333] text-[#f5f5f5] border border-[#2e2e2e]',
    ghost: 'bg-transparent hover:bg-[#333333] text-[#a3a3a3] hover:text-[#f5f5f5]',
  };
  const sizes = { sm: 'px-3 py-1.5 text-xs', md: 'px-4 py-2 text-sm' };
  return <button className={`rounded transition-all duration-150 ${variants[variant]} ${sizes[size]} ${className}`}>{children}</button>;
};
...
const ProgressBar = ({ value }) => (
  <div className="w-full h-1.5 bg-[#2a2a2a] rounded-full overflow-hidden">
    <div className="h-full bg-[#d97706] rounded-full transition-all" style={{ width: `${value}%` }} />
  </div>
);

const NavItem = ({ icon, label, active = false }) => (
  <a href="#" className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-all ${active ? 'bg-[rgba(217,119,6,0.15)] text-[#d97706]' : 'text-[#a3a3a3] hover:bg-[#333333] hover:text-[#f5f5f5]'}`}>
...
const CourseCard = ({ emoji, title, lessons, duration, progress }) => (
  <Card hoverable>
    <div className="aspect-video rounded-md bg-[#2a2a2a] mb-4 flex items-center justify-center text-4xl">{emoji}</div>
    ...
    {progress !== undefined ? (
      <div className="flex items-center gap-2"><ProgressBar value={progress} /><span className="text-xs text-[#737373]">{progress}%</span></div>
    ) : (
    ...
    <div className="flex min-h-screen bg-[#1a1a1a] text-[#f5f5f5] font-sans">
      ...
      <aside className="w-56 bg-[#1a1a1a] border-r border-[#2e2e2e] p-4 flex flex-col">
        ...
        <div className="pt-4 border-t border-[#2e2e2e]">
          <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-[#333333] cursor-pointer">
            ...
            <div><div className="text-sm text-[#f5f5f5]">Akram</div><div className="text-xs text-[#737373]">Pro Plan</div></div>
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 p-8 overflow-auto">
        <div className="max-w-5xl mx-auto">
          {/* Welcome */}
          <div className="mb-8">
            <h1 className="text-2xl font-bold text-[#f5f5f5] mb-2">Welcome back, Akram</h1>
            <p className="text-[#a3a3a3]">Continue where you left off or explore new courses</p>
          </div>
          ...
          <section className="mb-8">
            ...
            <Card hoverable className="flex gap-4">
              <div className="w-40 h-24 rounded-md bg-[#2a2a2a] flex items-center justify-center text-3xl">🐍</div>
              <div className="flex-1 flex flex-col justify-between">
                <div><Badge variant="accent">Continue</Badge><h3 className="text-[#f5f5f5] font-semibold mt-2">Python Fundamentals</h3><p className="text-sm text-[#a3a3a3]">Lesson 16: Working with Files</p></div>
                <div className="flex items-center gap-4"><ProgressBar value={65} /><span className="text-xs text-[#737373]">65%</span></div>
              </div>
              <div className="flex items-center"><Button variant="primary" size="sm">Resume</Button></div>
            </Card>
          </section>
          ...
          <section className="mb-8">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-[#f5f5f5]">In Progress</h2>
          ...
          <section className="mb-8">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-[#f5f5f5]">Recommended for You</h2>
              <Button variant="ghost" size="sm">View all →</Button>
            </div>
            <div className="grid grid-cols-3 gap-4">
