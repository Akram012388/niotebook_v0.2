  ...
  <Card hoverable onClick={() => onSelect(course)} className="flex gap-5">
    ...
    <div className="flex-1 flex flex-col justify-between py-1">
      <div>
        ...
        <h3 className="text-lg font-semibold text-[#f5f5f5] mb-1">{course.title}</h3>
        <p className="text-sm text-[#a3a3a3] line-clamp-2">{course.description}</p>
      </div>
      ...
      {course.progress !== undefined ? (
        <>
          <div className="text-right">
            ...
            <div className="text-lg font-semibold text-[#d97706]">{course.progress}%</div>
          </div>
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-[500px] bg-[#1a1a1a] border-l border-[#2e2e2e] overflow-auto">
        ...
        <div className="sticky top-0 bg-[#1a1a1a] border-b border-[#2e2e2e] p-6 flex items-start justify-between">
          <div>
            <Badge>{course.level}</Badge>
            <h2 className="text-xl font-bold text-[#f5f5f5] mt-2">{course.title}</h2>
          </div>
          <button onClick={onClose} className="text-[#737373] hover:text-[#f5f5f5] text-xl">✕</button>
        </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="text-center p-3 bg-[#232323] rounded-lg">
              <div className="text-lg font-semibold text-[#f5f5f5]">{course.lessons}</div>
              <div className="text-xs text-[#737373]">Lessons</div>
            </div>
            <div className="text-center p-3 bg-[#232323] rounded-lg">
              <div className="text-lg font-semibold text-[#f5f5f5]">{course.duration}</div>
              <div className="text-xs text-[#737373]">Duration</div>
            </div>
            <div className="text-center p-3 bg-[#232323] rounded-lg">
              <div className="text-lg font-semibold text-[#f5f5f5]">⭐ {course.rating}</div>
              <div className="text-xs text-[#737373]">Rating</div>
            </div>
          </div>
          ...
          {/* Description */}
          <div>
            <h3 className="text-sm font-semibold text-[#f5f5f5] mb-2">About this course</h3>
            <p className="text-sm text-[#a3a3a3] leading-relaxed">{course.description}</p>
          </div>

          {/* What you'll learn */}
          <div>
            <h3 className="text-sm font-semibold text-[#f5f5f5] mb-3">What you&apos;ll learn</h3>
          ...
          {/* Lessons Preview */}
          <div>
            <h3 className="text-sm font-semibold text-[#f5f5f5] mb-3">Course Content</h3>
            <div className="space-y-1">
              {[1, 2, 3, 4, 5].map(i => (
                <div key={i} className="flex items-center gap-3 p-3 rounded-lg hover:bg-[#232323] cursor-pointer transition-colors">
                  <div className="w-6 h-6 rounded-full bg-[#2a2a2a] flex items-center justify-center text-xs text-[#737373]">
                    {i}
                  </div>
                  <div className="flex-1">
                    <div className="text-sm text-[#f5f5f5]">Lesson {i}: Introduction to {course.title.split(' ')[0]}</div>
              ...
              <div className="text-center py-2">
                <Button variant="ghost" size="sm">Show all {course.lessons} lessons</Button>
              </div>
            </div>
          </div>
        </div>
  ...
  const courses = [
    {
      ...
      description: 'Master Python from scratch. Learn variables, data types, control flow, functions, and object-oriented programming with hands-on exercises.',
      level: 'Beginner',
      lessons: 24,
      duration: '4h 30m',
      students: 12500,
    ...
    },
    {
      ...
      description: 'Build modern web applications with React. Learn components, hooks, state management, and best practices for scalable apps.',
      level: 'Intermediate',
      lessons: 18,
      duration: '3h 15m',
      students: 8300,
    ...
    },
    {
      ...
      description: 'Learn SQL from the ground up. Write queries, join tables, aggregate data, and understand database design principles.',
      level: 'Beginner',
      lessons: 12,
      duration: '2h 45m',
      students: 15200,
    ...
    },
    {
      ...
      description: 'Create stunning visualizations with matplotlib, seaborn, and plotly. Turn data into compelling stories.',
      level: 'Intermediate',
      lessons: 15,
      duration: '2h 30m',
      students: 6400,
    ...
    },
    {
      ...
      description: 'Understand the fundamentals of ML. Learn about algorithms, model training, evaluation, and practical applications.',
      level: 'Advanced',
      lessons: 20,
      duration: '4h 00m',
      students: 9800,
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-[#a3a3a3]">Explore our library of interactive coding courses</p>
            </div>
            <SortDropdown />
          </div>
          ...
          <div className="space-y-4">
            {courses.map((course, i) => (
              <CourseCardLarge key={i} course={course} onSelect={setSelectedCourse} />
            ))}
          </div>
        </div>
      </main>
