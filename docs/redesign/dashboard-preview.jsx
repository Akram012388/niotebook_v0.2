...
const Card = ({ children, className = "", hoverable = false }) => (
  <div
    className={`bg-[#232323] border border-[#2e2e2e] rounded-lg p-4 ${hoverable ? "hover:border-[#404040] transition-all cursor-pointer" : ""} ${className}`}
  >
    {children}
  </div>
);
...
const Button = ({
  ...
  const variants = {
    ...
    secondary:
      "bg-neutral-800 hover:bg-neutral-700 text-gray-100 border border-neutral-700",
    ghost:
      "bg-transparent hover:bg-neutral-800 text-neutral-400 hover:text-gray-100",
  };
...
const NavItem = ({ icon, label, active = false }) => (
  <div
    className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm cursor-pointer transition-all ${active ? "bg-amber-900/20 text-amber-500" : "text-neutral-400 hover:bg-neutral-800 hover:text-gray-100"}`}
  >
    <span>{icon}</span>
    <span>{label}</span>
  </div>
...
const CourseCard = ({ emoji, title, lessons, duration, progress }) => (
  <Card hoverable>
    <div className="aspect-video rounded bg-neutral-800 mb-3 flex items-center justify-center text-3xl">
      {emoji}
    </div>
      ...
      <aside className="w-52 bg-neutral-900 border-r border-neutral-800 p-3 flex flex-col">
        ...
        <div className="pt-3 border-t border-neutral-800">
          <div className="flex items-center gap-2 p-2 rounded-lg hover:bg-neutral-800 cursor-pointer">
            ...
            <div>
              <div className="text-xs text-gray-100">Akram</div>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <section className="mb-6">
            ...
            <Card hoverable className="flex gap-4">
              <div className="w-32 h-20 rounded bg-neutral-800 flex items-center justify-center text-2xl shrink-0">
                🐍
              </div>
              <div className="flex-1 flex flex-col justify-between min-w-0">
                <div>
                  <Badge variant="accent">Continue</Badge>
                  <h3 className="text-gray-100 font-medium mt-1">
                    Python Fundamentals
                  </h3>
