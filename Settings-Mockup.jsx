...
const Button = ({
  ...
  return (
    <button
      className={`rounded transition-all ${variants[variant]} ${sizes[size]} ${className}`}
    >
      {children}
    </button>
  );
...
const Input = ({ label, type = "text", placeholder, value, helper }) => (
  <div className="space-y-1">
    ...
    <input
      ...
      defaultValue={value}
      className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-gray-900 text-sm placeholder-gray-400 focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500/20"
    />
...
const Toggle = ({ label, description, enabled = false }) => (
  <div className="flex items-center justify-between py-2">
    ...
    <button
      className={`relative w-10 h-5 rounded-full transition-colors ${enabled ? "bg-amber-600" : "bg-gray-300"}`}
    >
      <span
...
const Select = ({ label, options, value }) => (
  <div className="space-y-1">
    ...
    <select
      defaultValue={value}
      className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-gray-900 text-sm focus:border-amber-500 focus:outline-none cursor-pointer"
    >
      {options.map((opt) => (
...
const NavItem = ({ icon, label, active = false }) => (
  <div
    className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm cursor-pointer transition-all ${active ? "bg-amber-100 text-amber-700" : "text-gray-600 hover:bg-gray-100 hover:text-gray-900"}`}
  >
    <span>{icon}</span>
    <span>{label}</span>
  </div>
    ...
    <nav className="w-40 shrink-0 space-y-1">
      {items.map((item) => (
        <button
          ...
          className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-xs text-left transition-all ${active === item.id ? "bg-white text-gray-900 border border-gray-200 shadow-sm" : "text-gray-500 hover:text-gray-900"}`}
        >
          <span>{item.icon}</span>
...
const ProfileSection = () => (
  <div className="space-y-4">
    <div>
      ...
      <p className="text-xs text-gray-500">Manage your public profile</p>
    </div>
  ...
  <div className="space-y-4">
    ...
    <Card className="space-y-1">
      ...
      <Toggle
        ...
        description="Auto-explain runtime errors"
        enabled
      />
    </Card>
    <div className="flex justify-end">
...
const SubscriptionSection = () => (
  <div className="space-y-4">
    ...
    <Card className="space-y-2">
      ...
      {[
        { date: "Jan 15, 2026", amount: "$12.00" },
        { date: "Dec 15, 2025", amount: "$12.00" },
      ].map((inv, i) => (
        <div
          key={i}
        ...
        >
          <div>
            <div className="text-xs text-gray-900">{inv.date}</div>
            <div className="text-xs text-gray-500">Pro Monthly</div>
          </div>
          <div className="text-right">
            <div className="text-xs text-gray-900">{inv.amount}</div>
...
const SecuritySection = () => (
  <div className="space-y-4">
    ...
    <Card>
      ...
      <Toggle
        label="Enable 2FA"
        description="Require authenticator code when signing in"
      />
    </Card>
    <Card className="space-y-2">
      ...
      <div className="flex items-center justify-between py-2">
        <div className="flex items-center gap-2">
          <span>💻</span>
          <div>
            <div className="text-xs text-gray-900">MacBook Pro • Chrome</div>
      ...
      <div className="flex items-center justify-between py-2">
        <div className="flex items-center gap-2">
          <span>📱</span>
          <div>
            <div className="text-xs text-gray-900">iPhone 15 • Safari</div>
      ...
      <aside className="w-52 bg-white border-r border-gray-200 p-3 flex flex-col">
        ...
        <div className="pt-3 border-t border-gray-200">
          <div className="flex items-center gap-2 p-2 rounded-lg hover:bg-gray-100 cursor-pointer">
            ...
            <div>
              <div className="text-xs text-gray-900">Akram</div>
